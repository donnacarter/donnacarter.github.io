  ...
  class Picture < Liquid::Tag
    ...
    def render(context)
      ...
      html_attr = if markup[:html_attr]
        ...
        # accept single quotes and keep empty string
        Hash[ *markup[:html_attr].scan(/(?<attr>[^\s="]+)(?:=('|")(?<value>[^"]*)('|"))?\s?/).flatten ]
      else
        {}
      end
      ...
      generate_images(instance, site.source, site.dest, settings['source'], settings['output'], site.config['baseurl'])
      # instance.each { |key, source|
      #   puts key, source, site, site.source, site.dest, settings['source'], settings['output'], site.config["baseurl"]
    ...
    def generate_images(instances, site_source, site_dest, image_source, image_dest, baseurl)
      ...
      instances.each do |key, instance|
        file_path = File.join(site_source, image_source, instance[:src])
        unless cached_images[instance[:src]]
          raise 'Could not find ' + file_path unless File.exists?(file_path)
          digest = Digest::MD5.file(file_path).hexdigest.slice!(0..5)
          cached_images[instance[:src]] = { digest: digest }
        end
        ...
        expected_width = instance[:width] ? instance[:width].to_s : '*'
        expected_height = instance[:height] ? instance[:height].to_s : '*'
        digest = cached_images[instance[:src]][:digest]
        expected_file = "#{basename}-#{expected_width}by#{expected_height}-#{digest}#{ext}"
        dest_dir = File.join(site_dest, image_dest, image_dir)
        dir_match = Dir[File.join(dest_dir, expected_file)]
    ...
    def generate_image(image, desired_width, desired_height, destination, basename, digest, ext)
      ...
      gen_width = if desired_width
        desired_width.to_f
      elsif desired_height
        orig_ratio * desired_height.to_f
      else
        orig_width
      end
      gen_height = if desired_height
        desired_height.to_f
      elsif desired_width
        desired_width.to_f / orig_ratio
      else
        orig_height
      end
      ...
      gen_dest_file = File.join(destination, gen_name)

      # Generate resized files
      unless File.exists?(gen_dest_file)

        warn "Warning:".yellow + " #{image[:src]} is smaller than the requested output file. It will be resized without upscaling." if undersize
